...
<Card className="relative border border-white/10 hover:border-amber-400/20">
    <div className='absolute inset-0 bg-linear-to-br from-cyan-400/5 
    ...
    <CardContent className="flex flex-col gap-5">
        <div className='flex items-start justify-between gap-3'>
            <div className='flex items-center gap-3 '>
                <Avatar className="w-11 h-11 border border-white/10 shrink-0">
                    ...
                    <AvatarFallback className="bg-cyan-400/10 border border-cyan-400/20
            ...
            {yearsExp && (
                <Badge className="shrink-0 border-white/10 text-stone-500 text-xs"
                variant='outline'>
                    {yearsExp}+ years
        ...
        {categories?.length > 0 && (
            <div className='flex flex-wrap gap-1.5'>
                {categories.slice(0,4).map((cat)=>(
                    <span key={cat}
                    className='text-xs px-2.5 py-1 rounded-lg border border-cyan-400/20
                    bg-cyan-400/5 text-cyan-400'>
                        {CATEGORY_LABEL[cat] ?? cat}
                    </span>
                ))}
                {categories.length > 4 && (
            <span className='text-xs px-2.5 py-1 rounded-lg border border-white/10
            text-stone-600'>
        ...
        <div className='flex items-center justify-between gap-3'>
            <div className='flex flex-col gap-1'>
                <p className='text-lg font-serif leading-none bg-linear-to-br 
            ...
            variant='blue'
            size='sm'
            //className="shrink-0 border-cyan-400/20 text-cyan-400 hover:bg-cyan-400/10
