import { addDays, addMinutes, differenceInMinutes, format, isAfter, isBefore, isToday, isTomorrow, milliseconds, set } from "date-fns";
import { minutesInDay } from "date-fns/constants";

//"Sun,Mar 24,2021"-used in appointment cards
export function formatDate(iso){
    return format(new Date(iso),"EEE,MMM d,yyyy");
}

//"Monday,March 24,2023"-used in booking confirm card
export function formatDateFull(date){
    return format(new Date(date),"EEEE,MMMM d,yyyy");
}

//"9:30 pm"-used anywhere a time-only string is needed
export function formatTime(date){
    return format(new Date(date),"h:mm a");
}

//"2h 45min" or "30mins"-used to show appointment cards to show session length
...
//bottom is always "MMM d" (e.g. Nov 26) regardless of which branch.
